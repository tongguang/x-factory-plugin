...
function configPath() {
  ...
  return import_node_path.default.join(import_node_os.default.homedir(), ".factory", "plugin-config", "image-gen", "config.json");
}
var TEMPLATE = `{
  "baseUrl": "",
  "apiKey": "",
...
async function loadConfig(file = configPath()) {
  if (!(0, import_node_fs.existsSync)(file)) {
    await (0, import_promises.mkdir)(import_node_path.default.dirname(file), { recursive: true });
    await (0, import_promises.writeFile)(file, TEMPLATE, { encoding: "utf8", flag: "wx", mode: 384 }).catch((err) => {
  ...
  let raw;
  try {
    raw = JSON.parse((await (0, import_promises.readFile)(file, "utf8")).replace(/^\uFEFF/, ""));
  } catch {
  ...
  const timeoutMs = typeof obj.timeoutMs === "number" && obj.timeoutMs > 0 ? Math.floor(obj.timeoutMs) : DEFAULT_TIMEOUT_MS;
  const missing = [];
...
async function writeBuffer(outputDir, buf) {
  ...
  const full = import_node_path2.default.join(outputDir, `img-${(0, import_node_crypto.randomUUID)()}${ext}`);
  await (0, import_promises2.writeFile)(full, buf, { flag: "wx" });
  return full;
}
async function saveBase64Image(outputDir, b64) {
...
async function saveImageFromUrl(outputDir, url, timeoutMs) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
...
async function post(config, endpoint, body) {
  ...
  try {
    ...
    if (err instanceof Error && err.name === "TimeoutError" || /timed out|timeout/i.test(msg)) {
      throw new ApiError(
        `\u8BF7\u6C42\u8D85\u65F6\uFF08>${Math.round(config.timeoutMs / 1e3)} \u79D2\uFF09\u3002\u7ED3\u679C\u672A\u77E5\uFF0C\u672A\u81EA\u52A8\u91CD\u8BD5\u4EE5\u907F\u514D\u91CD\u590D\u8BA1\u8D39\uFF1B\u53EF\u8C03\u5927 config.json \u7684 timeoutMs\u3002`
      );
    }
...
async function saveImageItem(outputDir, item, timeoutMs) {
...
async function requestGeneration(config, params) {
...
async function executeImages(args, config, outputDir, request) {
  const result = { paths: [], requested: args.count, prompt: args.prompt };
  try {
    ...
    const message = `[images] ${err instanceof Error ? err.message : String(err)}`.replaceAll(config.apiKey, "[\u5DF2\u9690\u85CF]");
    if (result.paths.length) throw new PartialGenerationError({ ...result, error: message });
    throw new ApiError(message, err instanceof ApiError ? err.status : void 0);
  }
}
async function readReferenceImage(imagePath) {
  if (!import_node_path3.default.isAbsolute(imagePath)) {
    throw new InputError(`\u53C2\u8003\u56FE\u5FC5\u987B\u4F7F\u7528\u7EDD\u5BF9\u8DEF\u5F84\uFF1A${imagePath}`);
  }
  const ext = import_node_path3.default.extname(imagePath).toLowerCase();
  ...
  let info;
  try {
    info = await (0, import_promises3.stat)(imagePath);
  } catch {
...
async function executeGenerateImage(args, config, outputDir = defaultOutputDir()) {
...
async function main() {
  ...
  const content = await (0, import_promises4.readFile)(values.input, "utf8");
  let input;
  try {
  ...
  const outputDir = values["output-dir"] ? import_node_path4.default.resolve(values["output-dir"]) : defaultOutputDir();
  const result = command === "generate" ? await executeGenerateImage(external_exports.object(generateImageSchema).parse(input), void 0, outputDir) : await executeEditImage(external_exports.object(editImageSchema).parse(input), void 0, outputDir);
  console.log(JSON.stringify(result));
}
main().catch((error) => {
  if (error instanceof PartialGenerationError) console.log(JSON.stringify(error.result));
  const message = error instanceof external_exports.ZodError ? `\u8BF7\u6C42\u53C2\u6570\u65E0\u6548\uFF1A${error.issues.map((issue) => `${issue.path.join(".") || "\u8BF7\u6C42"}: ${issue.message}`).join("\uFF1B")}` : error instanceof Error ? error.message : String(error);
