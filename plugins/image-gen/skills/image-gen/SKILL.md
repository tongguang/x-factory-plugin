---
name: image-gen
description: 使用 GPT Image 2.5 生成或编辑图片。当用户要求生成图片、画图、文生图，或修改一张已有本地图片时使用。通过插件自带 Node.js CLI 生成图片，保存到当前项目并展示。
---
...
- 这个 CLI 的 `edit` 只有一张编辑目标图；不要把仅用于参考风格、而不应修改的图片当作编辑目标。

## 提示词

- 保留用户指定的主体、风格、构图、用途和限制。要求已具体时只整理表达；要求笼统时，可补充有助于成图的画面细节，不自行增加人物、品牌或文字。
- 图片中需要文字时，写明必须逐字呈现的内容与位置；编辑时说明要改的部分，并明确应保留的主体、构图、文字等。后续修改继续写明这些不变项。
- 用户要求透明贴图或去背时，要求真正透明的背景及干净的主体边缘。不要在提示词中加入棋盘格、纯色底或其他模拟透明的背景。
...
3. `size` 仅在用户明确要求时填写；可用 `"auto"` 或 `"宽x高"`，如 `"1024x1024"`、`"2048x1152"`、`"3840x2160"`。宽高均须为 16 的倍数且不超过 3840；长短边比例不超过 3:1；总像素须在 655,360～8,294,400 之间。超过 3,686,400 像素（2560×1440）的自定义尺寸属实验性。CLI 会在请求前校验，不自行修改用户指定的尺寸。
4. `edit` 额外传入 `imagePath`，必须是存在的本地绝对路径，格式限 PNG / JPG / JPEG / WebP，文件非空且严格小于 50,000,000 字节。
...
- 对透明成品，如可检查文件，确认 PNG 中确有 Alpha 及透明像素；无法检查时只说明已请求透明背景，不保证实际透明效果。
...
- 插件只支持 GPT Image 2.5 Flare / Sunburst 及其 `2026-09-08` 快照，CLI 在请求前校验模型和尺寸。文生图请求 `/images/generations`，参考图编辑请求 `/images/edits`；路径追加到配置中的 `baseUrl`，不自动添加 `/v1`。
- 自定义 `baseUrl` 需要支持对应的 OpenAI Images API 语义；若服务报错，如实说明，不自动更换服务或模型。
- 不主动读取或展示配置文件内容，不输出密钥，也不猜测服务地址、密钥或模型名。配置问题引导用户填写 `baseUrl`、`apiKey`、`model`，不要自行更改用户的服务或模型选择。
- 编辑会将参考图上传到用户配置的图片服务。输出保存为新文件，不覆盖参考图。
- 输出按文件头识别 PNG、JPEG、WebP、GIF，单张本地上限 64 MiB；格式识别优先于下载响应的 `Content-Type`。无法识别的内容会报错，不把 HTML 或文本当作图片展示。轻量格式识别不代表完整解码验证。
